...
public class Octagon extends JPanel implements Object {
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Calculate the coordinates of the points of the octagon
        for (int i = 0; i < 8; i++) {
        ...
        int[] xShadowPoints = new int[8];
        int[] yShadowPoints = new int[8];
        for (int i = 0; i < 8; i++) {
            xShadowPoints[i] = xPoints[i] + shadowOffsetX;
            yShadowPoints[i] = yPoints[i] + shadowOffsetY;
        }
        g.fillPolygon(xShadowPoints, yShadowPoints, 8);
    ...
    @Override
    public void draw() {
        JFrame frame = new JFrame("Octagon Drawing Example");
